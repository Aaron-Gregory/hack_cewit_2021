...
def localize(xray_img):
    ...
    onto xray_img showing where anomalies are, but we don't have
    ...
    w = img_rgb.size[0]
    img.rectangle([(int(w * rect[0]), int(w * rect[1])), (int(w * rect[2]), int(w * rect[3]))], fill =None, outline ="#ff3333")
    return img_rgb
...
def Process_xray(xray_img, global_check, locate_anomalies, results_text, result_img):
    """
    ...
    if normalcy_code == 1: # indicates healthy xray
        ...
        if pneumonia_code == 0: # models agree
            results_text("Healthy")
        else: # probably a false positive
            results_text("Pneumonia unlikely")
        ...
        if pneumonia_code == 0: # probably a false negative
            results_text("Pneumonia likely")
        elif pneumonia_code == 1: # not covid
            results_text("Viral pneumonia detected")
        else: # not covid
            results_text("COVID-19 detected")


if __name__ == "__main__":
    # a bunch of functions to use as callbacks
    def f1(x):
        print("Global_Check", x)
    
    def f2(x):
        print("Locate_anomalies", x)
    
    def f3(x):
        print("results_text", x)
